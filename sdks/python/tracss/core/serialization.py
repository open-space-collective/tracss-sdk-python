...
def _compute_requires_conversion(type_: typing.Any, seen: typing.Set[typing.Any]) -> bool:
    ...
    if (
        inspect.isclass(clean_type) and issubclass(clean_type, pydantic.BaseModel)
    ) or typing_extensions.is_typeddict(clean_type):
        annotations = _get_cached_type_hints(clean_type)
        if _get_alias_to_field_name(annotations):
            return True
        return any(
            _compute_requires_conversion(hint, seen) for hint in annotations.values()
        )

    # Containers / unions: recurse into the type arguments (List/Set/Sequence/Dict/Union/etc.).
    return any(
        _compute_requires_conversion(arg, seen)
        for arg in typing_extensions.get_args(clean_type)
    )
